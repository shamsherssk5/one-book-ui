...
const Notes = ({ slectedSubMenu, data, setData, currentCustomer, setToken }) => {
    ...
    const handleDelete = async (note) => {
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/delete-note', { 'id': note.id }, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    const handleSaves = async () => {
        ...
        noteData.note_time=d.getFullYear().toString() + "-" + (d.getMonth() + 1).toString() + "-" + d.getDate().toString()+" "+d.getHours().toString() + ":" + d.getMinutes().toString() + ":" + d.getSeconds().toString();
        if (SubmitButton === "ADD NOTE") {
            setSubmitButton("...Saving")
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/create-note', noteData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
            ...
            setSubmitButton("...Saving")
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/update-note', noteData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
    ...
    useEffect(async () => {
        ...
        if (currentCustomer && currentCustomer.notes && currentCustomer.notes.length > 0) return;
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/suppliers/sup-notes?ID=' + currentCustomer.supID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
                setData((prev) => {
                    let updatedData = prev.filter(cust => {
                        if (cust.supID === currentCustomer.supID) {
                            cust["notes"] = res.data.filter(d=>d.note && d.note.length>0)
                        }
                        return cust;
                    })
                    return updatedData;
    ...
    return (
        ...
        <div className='notes-container'>
            <div className='notes-adder-container'>
                ...
                <input type="text" className='notes-add-input' value={noteData.note} placeholder='Enter Here...' onChange={(e) => setNoteData({ ...noteData, note: e.target.value })}></input>
                <button id="next" className='notes-add-button' onClick={handleSaves}>{SubmitButton}</button>
            </div>
                                ...
                                <tbody>
                                    <tr>
                                        <td align='left'>
                                            ...
                                            <p className='notes-data-info'>created by {h.created_by} on {moment(h.note_time.replace('T',' ').replace('Z','')).format(currentUser.dateFormat+ (currentUser.timeFormat==="12 Hrs" ? " hh:mm A":" HH:mm"))}</p>
                                        </td>
                                        <td align='right'>
