...
const Reminders = ({ slectedSubMenu, data, setData, currentCustomer, setToken, contacts }) => {
    ...
    const handleDelete = async (rem) => {
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/delete-reminder', { 'id': rem.id }, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    const handleSaves = async () => {
        ...
        if (SubmitButton === "ADD") {
            ...
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/create-reminder', reminderData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
            ...
            setSubmitButton("...Saving")
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/update-reminder', reminderData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
    ...
    useEffect(async () => {
        ...
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/suppliers/sup-reminders?ID=' + currentCustomer.supID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
