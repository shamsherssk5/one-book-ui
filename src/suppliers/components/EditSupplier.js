...
const EditSupplier = ({ setRightContent, setToken, rightContent,industryType, currentCustomer, setData,contacts }) => {
    ...
    useEffect(async () => {
        ...
        if (SubmitButton === "...Saving") {
            ...
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/suppliers/update-supplier?timeZone='+currentUser.timeZone, data, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
                    toast.success("Supplier updated successfully");
                    formData.history.unshift({"moduleID":formData.supID,"action":"Edited","dateAndTime":new Date(new Date().toLocaleString("en-US", {timeZone: currentUser.timeZone})).toLocaleString(),"name":formData.assignee});
                    setData(prev => {
    ...
    useEffect(async ()=>{
        ...
        setOpenFinance(false);
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/suppliers/sup-financial?ID=' + currentCustomer.supID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
