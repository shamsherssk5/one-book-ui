...
const UserListItems = ({currentItems, editRole, setToken, setUsersList}) => {
    ...
    const handleBlockUnBlock=async (user)=>{  
        document.body.click();
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/settings/blockUser', user, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
        .then((res) => {
            if (res.data.error) {
                setToken(undefined);
            }
            ...
            toast.success("User "+((user.active=='yes')?'UnBlocked':'Blocked')+" Sucessfully");
    ...
    const handleDelete=async (user)=>{
        ...
        document.body.click();
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/settings/deleteUser', user, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
        .then((res) => {
            if (res.data.error) {
                setToken(undefined);
            }
                    ...
                    <table className="list-view-table">
                        ...
                        <tbody className="tbody-class">
                            {currentItems && currentItems.map((user, index) => {
                                return (<>
                                    <tr key={index} className="task-list-row-container users-list-tr" >
                                                ...
                                                overlay={(
                                                    <Popover>
                                                        <div className={Style.popup}>
                                                            <p className="p-leave" onClick={()=>editRole(user)}>
                                                                Edit Role
                                                            </p>
                                                            <p className={Style.delete} onClick={()=>handleDelete(user)}>
                                                                Delete
                                                            </p>
                                                            <p className={Style.delete} onClick={()=>handleBlockUnBlock(user)}>
