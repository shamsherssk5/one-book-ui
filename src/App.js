...
import { HashRouter as Router, Routes, Route, Link, HashRouter, BrowserRouter, useLocation } from "react-router-dom";
...
function App() {
  ...
  const [loader, loading] = useState({ visibility: false, message: "Loading..." })
  ...
  useEffect(async () => {
    ...
    if (tok && user) {
      ...
      await axios.get(process.env.REACT_APP_API_ENDPOINT + '/settings/getLandingPageData?userID=' + user.id, { headers: { "Authorization": tok } })
        .then((res) => {
          if (res.data.error) {
            setToken(undefined);
          }
  ...
  useEffect(async () => {
    ...
    if (tok && user) {
      ...
      await axios.get(process.env.REACT_APP_API_ENDPOINT + '/settings/fetchMenus', { headers: { "Authorization": tok } })
        .then((res) => {
          if (res.data.error) {
            setToken(undefined);
          }
  ...
  const checkNavigation=(menu)=>{
    ...
    if(!clickedMenu.isClickable==0)
      navigate(clickedMenu.url.toLowerCase());
  }
      ...
      <div className="App myscrollbar">
          ...
          <div className="header-top-part">
            <div className="text-part">
              <div className="wh-bg-div">
                <div className="wh-bg" style={{ backgroundColor: orgData && orgData.filename ? "transparent" : "#fff" }}>
              ...
              <div className="non-wh-bg">
                <div style={{ width: "47%", height: "100%" }}>
                  <span>Organization<strong>{(orgData && orgData.crm) || 'Artefact Exhibition LLC'}</strong></span>
                </div>
            ...
            <div className="r-text-part">
              <span>
                <marquee>
                  OneBook Customers, Suppliers, Phone Book Modules are rolled out |  Materal Request, Products and Materials Modules are under development
                </marquee>
              </span>
            ...
            <div class="links-container">
              {
                [...new Set(menuList.map(m => m.menu))].map((menu, index) =>
                  <Dropdown id="custome-menu">
                    ...
                    {menuList.filter(men => men.menu === menu && men.submenu.length>0).length>0 && <Dropdown.Menu id="list-menu">
                      {menuList.filter(men => men.menu === menu).map((men, index) => <Dropdown.Item id="list-a-item" onClick={() => navigate(men.url.toLowerCase())} eventKey={index}>{men.submenu}</Dropdown.Item>)}
                    </Dropdown.Menu>}
                  </Dropdown>
                )
              }
            ...
            <div class="images-container">
              ...
              <OverlayTrigger
                ...
                <div className="user-holder">
                  {(currentUser && currentUser.username) ? <span><Avatar size="1.7vw" title={currentUser.username} round="50%" textSizeRatio={3} textMarginRatio={0.15} name={currentUser.username} /></span>
        ...
        <div className="content-part">
          <div className="main-content-div">
            ...
            <Routes>
              <Route exact path='/' element={<Structure2 />} />
              <Route path='/suppliers' element={<SuppliersHome setToken={setToken} loading={loading} />} />
              <Route path='/materialrequest' element={<MaterialRequestHome />} />
              <Route path='/phonebook' element={<PhonebookHome loading={loading} setToken={setToken} />} />
              <Route path='/tasks' element={<Template1Tasks loading={loading} setToken={setToken} />} />
              <Route path='/customers' element={<CustomersHome setToken={setToken} loading={loading} />} />
              <Route path='/settings' element={<Settings setToken={setToken} orgData={orgData} setOrgdData={setOrgdData} loading={loading} />} />
            </Routes>
          ...
          <div className="notification-bar">
            <div className="notif-table-container">
              <div className="notif-images-container">
                <span><img className="img-size" src={Bell} /></span>
