...
const Notes = ({ slectedSubMenu, data, setData, currentCustomer, setToken }) => {
    ...
    const handleDelete = async (note) => {
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/delete-note', { 'id': note.id }, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    const handleSaves = async () => {
        ...
        if (SubmitButton === "ADD NOTE") {
            ...
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/create-note', noteData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
            ...
            setSubmitButton("...Saving")
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/update-note', noteData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
    ...
    useEffect(async () => {
        ...
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/customers/cust-notes?ID=' + currentCustomer.custID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
