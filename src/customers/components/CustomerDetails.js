...
const CustomerDetails = ({ rightContent, currentCustomer, setToken, setData, contacts, setContacts }) => {
    ...
    useEffect(async () => {
        ...
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/files/attachments?ID=customer' + currentCustomer.custID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    useEffect(async () => {
        ...
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/common/history?ID=customer' + currentCustomer.custID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    const handleDeleteContact = async (contact) => {
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/delete-contact', { 'id': contact.id }, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    useEffect(async () => {
        if (SubmitButton === "...Saving") {
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/create-contact', formData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
            ...
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/update-contact', formData, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
    ...
    return (
        ...
        <div className="task-details-box" ref={ref}>
            ...
            <Gap></Gap>
            {currentCustomer && currentCustomer.attachments && <FileUploaderListViewer isView={true} setToken={setToken} data={currentCustomer.attachments} handleUpload={handleUpload} handleDelete={handleDelete} module="customer" id={"customer" + currentCustomer.custID} />}
            <Gap />
