...
const CreateCustomer = ({ setRightContent,industryType,setindustryType ,rightContent, setToken, setData,contacts }) => {
    ...
    useEffect(async () => {
        ...
        if (SubmitButton === "...Saving") {
            ...
            await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/create-customer?timeZone='+currentUser.timeZone, data, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
