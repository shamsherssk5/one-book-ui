...
const CustomersHome = ({ setToken, loading }) => {
    ...
    const handleAddToCudstomer=async ()=>{
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/add-to-supplier?timeZone='+currentUser.timeZone,{orgID:currentUser.orgID, custID:currentCustomer.custID, assignee:currentUser.username}, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    useEffect(async () => {
        ...
        await axios.get(process.env.REACT_APP_API_ENDPOINT + '/customers/customers-list?orgID=' + currentUser.orgID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
            .then((res) => {
                if (res.data.error) {
                    setToken(undefined);
                }
    ...
    const handleAddToMenu=async (menu)=>{
        ...
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/update-customer-category', d, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                    .then((res) => {
                        if (res.data.error) {
                            setToken(undefined);
                        }
    ...
    const handleDeleteSelected=async (IDs) => {
        ...
        await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/delete-customer', {'IDs':IDs}, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                    .then((res) => {
                        if (res.data.error) {
                            setToken(undefined);
                        }
                        setOriginalData(prev => {
                            let updatedData = prev.filter(cust => !IDs.toString().split(",").includes(cust.custID.toString()))
                            return updatedData;
                        })
        ...
        <div className="main-left-right-div-container">
                                ...
                                <div className="popup">
                                    {homeView === "customersList" &&
                                        <>
                                            ...
                                            <p onClick={() => {document.body.click();}}>Merge Customers</p>
                                    ...
                                    {homeView === "customerDetail" &&
                                        <>
                                            ...
                                            <p onClick={() => {handleAddToCudstomer()}}>Copy to Supplier List</p>
                                            ...
                                            <p className="popup-danger" onClick={() => {handleDeleteSelected(currentCustomer.custID)}}> Delete</p>
                                        </>
                                    }   
                                </div>
                            </Popover>
                ...
                {homeView === "customersList" &&
                    <div className="left-div-content customers">
                        ...
                        <div className='customers-container'>
                            <CustomerPaginatedItems setToken={setToken} deleteTrigger={deleteTrigger} setDeleteTrigger={setDeleteTrigger}  handleDeleteSelected= {handleDeleteSelected} setRightContent={setRightContent} itemsPerPage={itemsPerPage} items={data} updateData={setOriginalData} setItemsPerPage={setItemsPerPage} setHomeView={setHomeView} setCurrentCustomer={setCurrentCustomer} addNote={addNote} setNoteClicked={setNoteClicked} setSelectedSubMenu={setSelectedSubMenu} addToMenu={addToMenu} setAddToMenu={setAddToMenu} />
                        </div>
                    </div>
                }
                {
                    ...
                    <div className="left-div-content customers">
                        ...
                        <div className='customers-numbers-summar-container' style={{ borderBottom: "0.5px solid #239BCF" }}>
                            <div className='customer-summary-menu-container'>
                                {
                                    ['Summary', 'Estimations', 'Quotes', 'Projects', 'Invoices', 'Notes', 'Reminders'].map(menu =>
            ...
            <div className="main-right-div">
                ...
                <div className="right-div-content">
                    ...
                    <CreateCustomer setRightContent={setRightContent} industryType={industryType} setindustryType={setindustryType} TypesetToken={setToken} rightContent={rightContent} contacts={contacts} setData={setOriginalData} />
                    <CreateContact setToken={setToken} rightContent={rightContent} contacts={contacts} setContacts={setContacts}/>
                    <CustomerDetails rightContent={rightContent} currentCustomer={currentCustomer} setData={setOriginalData} contacts={contacts} setContacts={setContacts}/>
                    <EditCustomer setRightContent={setRightContent} industryType={industryType} setToken={setToken} contacts={contacts} rightContent={rightContent} currentCustomer={currentCustomer} setData={setOriginalData} />
                </div>
            </div>
        </div>
    );
