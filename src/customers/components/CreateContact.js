...
const CreateContact = ({ rightContent, contacts, setContacts, setToken }) => {
    ...
    useEffect(async () => {
        ...
        if (rightContent === "CreateContact" || rightContent === "Create" || rightContent==="Company Info") {
            await axios.get(process.env.REACT_APP_API_ENDPOINT + '/customers/customers-contact-list?orgID=' + currentUser.orgID, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                .then((res) => {
                    if (res.data.error) {
                        setToken(undefined);
                    }
