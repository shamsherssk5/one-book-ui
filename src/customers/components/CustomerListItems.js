...
const CustomerListItems = ({ setToken,setRightContent, deleteTrigger,setDeleteTrigger,handleDeleteSelected,currentItems, addToMenu, setAddToMenu, setNoteClicked, updateData, setSelectedSubMenu, setHomeView, setCurrentCustomer, addNote }) => {
    ...
    useEffect(() => {
        if (addNote) {
            let checkedItem = currentItems.filter(item => document.getElementById(item.custID).checked);
    ...
    useEffect(async () => {
        if (addToMenu) {
            ...
            if (checkedItem.length === 0) {
                ...
                let data={
                    custIDs:checkedItem.length===1?checkedItem.map(cust=>cust.custID)[0]:checkedItem.map(cust=>cust.custID).join(),
                    category:addToMenu
                }
                await axios.post(process.env.REACT_APP_API_ENDPOINT + '/customers/update-customer-category', data, { headers: { "Authorization": window.sessionStorage.getItem("token") } })
                    .then((res) => {
                        if (res.data.error) {
                            setToken(undefined);
                        }
    ...
    useEffect(()=>{
        if(deleteTrigger){
            ...
            if (checkedItem.length === 0) {
                ...
                let IDs=checkedItem.length===1?checkedItem.map(cust=>cust.custID)[0]:checkedItem.map(cust=>cust.custID).join();
    ...
    const handleOnchange = (e) => {
        let isAllChecked = currentItems.every(d => document.getElementById(d.custID).checked);
        if (isAllChecked) {
                            ...
                            {currentItems.map((cust, index) => {
                                return (<>
                                    <tr key={index} className="task-list-row-container" onClick={(e) => handleCustomerClick(e, cust)}>
                                        <td width="3.1%" className="chk-box-container-td"><input className="chkbx" name={cust.custID} id={cust.custID} type="checkbox" onChange={handleOnchange} /></td>
                                        ...
                                        <td width="11.2%" >{moment(cust.created_date).format(currentUser.dateFormat)}</td>
                                        <td width="18.1%" >{cust.managers && cust.managers.length > 0 && cust.managers.split(",").map(d => <><span style={{ marginRight: "0.3vw" }}></span><Avatar size="1.75vw" title={d} round="50%" textSizeRatio={3} textMarginRatio={0.15} name={d} /></>)}</td>
                                    </tr>
