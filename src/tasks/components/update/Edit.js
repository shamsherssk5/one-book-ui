...
const Edit=(props)=>{
    let {task,setData,projects,usersList,departments,currentUser,setToken}=props;
    ...
    const formValidation = () => {
        ...
        return keys.some((key) => { console.log(formData[key]);return formData[key].toString().trim().length == 0 });
    }
    const handleFormSubmit = (e) => {
        ...
        axios.post(process.env.REACT_APP_API_ENDPOINT + '/tasks/editTask', formData,{ headers: {"Authorization" : window.sessionStorage.getItem("token")} })
        .then((res) => {
            if(res.data.error){
                setToken(undefined);
            }
            setData((prevState) => {
                ...
                updatedTask.userNames=assignTo.map(user=>user.userName).join(',');
                ...
                let updatedData=prevState.tasks.filter(t=>{
                    if(t.id===task.id){
                        t=updatedTask;
                        t.history.unshift({"moduleID":formData.id,"action":"Edited","dateAndTime":d.toLocaleString(),"name":formData.assignee});
                    }
                    return t;
                })
                return { tasks: updatedData, rightContent: "Details", currentTask:updatedTask, category: prevState.category, isScrollButtonVisible: prevState.isScrollButtonVisible }
            });
    ...
    useEffect(()=>{  
        ...
        setAssinTo(usersList.filter(user=>task.userNames && task.userNames.includes(user.userName)));
                        ...
                        <div className="form-container">
                            ...
                            <fieldset>
                                ...
                                <Multiselect
                                    ...
                                    selectedValues={usersList.filter(user=>task.userNames.includes(user.userName))}
                                />
                            </fieldset>
                            {!isCustomDate &&
                                <fieldset>
                            ...
                            <div className="submit-button-container">
                                <input className="submit-button" type="button" value={submitValue} onClick={handleFormSubmit} />
                            </div>
                        </div>
                    </form>
                </div>
